...
const SubProjectList = (props: any) => {
  ...
  return (
    <Container>
        ...
        {isLoading ? <Row style={{flex:1,justifyContent:'center', alignItems:'center'}}><Spinner/></Row> : (error && !data) ? <div>{`Ошибка - ${error}`}</div> : 
        <Row> 
            {subProjects.length &&
            <Col size={2} sizeXL={2} sizeL={2} sizeM={2} sizeS={2}>
                 <Header title={'Объекты'} />
                 {subProjects.map((it:any)=> (<ButtonSubProject key={it.id} id={it.id} title={it.title} active={it.id===active} onClick={()=>onChangeSubProject(it.id, it.title)} />))}
            </Col>}
